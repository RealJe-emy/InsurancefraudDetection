...
def predictRouteClient():
    ...
    try:
        ...
        # Assuming df is defined from above code block
        ...
        results = []
        # Add policy numbers to output CSV
...
def add_policy_numbers_to_csv(policy_numbers, output_path):
    """Add policy numbers to the prediction output CSV file"""
    # Read the existing CSV file
    df = pd.read_csv(output_path)

    # Make sure the lengths match
    if len(df) != len(policy_numbers):
        policy_numbers = policy_numbers[:len(df)]  # Truncate if necessary

    # Add the policy numbers as a new column
